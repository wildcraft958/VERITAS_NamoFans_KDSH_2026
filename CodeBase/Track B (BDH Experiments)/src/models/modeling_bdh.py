...
# CacheLayerMixin may not exist in older transformers versions
try:
    from transformers.cache_utils import CacheLayerMixin
except ImportError:
    # Fallback for older transformers versions
    class CacheLayerMixin:
        """Fallback CacheLayerMixin for transformers < 4.38"""
        pass
...
class BDHCache(Cache):
    ...
    def __init__(
        ...
        # Skip parent __init__ entirely - we manage our own state
        # This avoids all transformers version compatibility issues
        self.layers: List[BDHCacheLayer] = []
        self.config = config
        self.dtype = dtype
        self._device = device
    ...
    def update(
        ...
        """
        Required by transformers Cache interface.
        BDH doesn't use traditional K/V cache, so we just track sequence length.
        """
        # Ensure layer exists
        while len(self.layers) <= layer_idx:
            self.layers.append(BDHCacheLayer())
        
        # Track sequence length (only on first layer to avoid double counting)
        if layer_idx == 0:
    ...
    def get_max_length(self) -> Optional[int]:
        """Return max cache length (None for infinite)."""
        return None
...
class BDHModel(BDHPreTrainedModel):
    ...
    def __init__(self, config: BDHConfig):
        ...
        if getattr(config, 'bdh_attn_implementation', 'bdh_recurrent') == "bdh_recurrent":
            self.attn = BDHRecurrentAttention(config)
        elif getattr(config, 'bdh_attn_implementation', 'bdh_recurrent') == "bdh_parallel":
            self.attn = BDHParallelAttention(config)
        else:
            raise ValueError(
                f"Unsupported attn_implementation {getattr(config, 'bdh_attn_implementation', 'bdh_recurrent')}"
            )
